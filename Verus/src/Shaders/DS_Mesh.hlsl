...
ConstantBuffer<UB_PerMaterial> g_ubPerMaterial : register(b0, space1);
ConstantBuffer<UB_PerMesh>     g_ubPerMesh     : register(b0, space2);
ConstantBuffer<UB_Skinning>    g_ubSkinning    : register(b0, space3);
VK_PUSH_CONSTANT
...
VSO mainVS(VSI si)
{
	...
	const float2 intactTc0 = DequantizeUsingDeq2D(si.tc0.xy, g_ubPerMesh._tc0DeqScaleBias.xy, g_ubPerMesh._tc0DeqScaleBias.zw);
	...
	float3 binWV;
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			const mataff matBone = g_ubSkinning._vMatBones[indices[i]];
		...
		const mataff matBone = g_ubSkinning._vMatBones[index];
...
_DS_FSO mainFS(VSO si)
{
...
#ifdef DEF_DEPTH
	so.color = si.depth.x / si.depth.y;
#ifdef DEF_CLIP
	...
	// <Albedo>
	const float texAlbedoEnable = ceil(g_ubPerMaterial._texEnableAlbedo.a);
	float4 rawAlbedo = g_texAlbedo.Sample(g_samAlbedo, si.tc0*texAlbedoEnable);
	rawAlbedo.rgb = lerp(g_ubPerMaterial._texEnableAlbedo.rgb, rawAlbedo.rgb, g_ubPerMaterial._texEnableAlbedo.a);
	const float2 alpha_spec = AlphaSwitch(rawAlbedo, si.tc0, g_ubPerMaterial._ssb_as.zw);

	const float emitAlpha = PickAlpha(rawAlbedo.rgb, g_ubPerMaterial._emitPick, 16.0);
	const float emitXAlpha = PickAlpha(rawAlbedo.rgb, g_ubPerMaterial._emitXPick, 16.0);
	const float eyeAlpha = PickAlphaRound(g_ubPerMaterial._eyePick, si.tc0);
	const float glossXAlpha = PickAlpha(rawAlbedo.rgb, g_ubPerMaterial._glossXPick, 32.0);
	const float hairAlpha = round(PickAlpha(rawAlbedo.rgb, g_ubPerMaterial._hairPick, 16.0));
	const float metalAlpha = PickAlpha(rawAlbedo.rgb, g_ubPerMaterial._metalPick, 16.0);
	const float skinAlpha = PickAlpha(rawAlbedo.rgb, g_ubPerMaterial._skinPick, 16.0);
	const float userAlpha = PickAlphaHue(rawAlbedo.rgb, g_ubPerMaterial._userPick, 32.0);

	// Add details:
#if 0
	const float3 colorDetail = (g_texDetail.Sample(g_samDetail, frac(si.tc0*g_ubPerMaterial._ds_scale.zw)).rgb - 0.5)*
		g_ubPerMaterial._ds_scale.x + 0.5;
	rawAlbedo.rgb = rawAlbedo.rgb*colorDetail*2.0;
#endif

	const float grey = Greyscale(rawAlbedo.rgb);
	rawAlbedo.rgb = lerp(rawAlbedo.rgb, Overlay(grey, si.color0.rgb), userAlpha*si.color0.a);
	// </Albedo>

	// <Normal>
	const float texNormalEnable = ceil(g_ubPerMaterial._texEnableNormal.a);
	float4 rawNormal = g_texNormal.Sample(g_samNormal, si.tc0*texNormalEnable);
	rawNormal = lerp(g_ubPerMaterial._texEnableNormal.rgbr, rawNormal, g_ubPerMaterial._texEnableNormal.a);
	...
	const float3 anisoWV = normalize(mul(cross(normalTBN, cross(g_ubPerMaterial._hairParams.xyz, normalTBN)), matFromTBN));
	// Almost solid hair color is used for picking.
	// Final albedo is calculated using specualar component.
	const float3 hairAlbedo = Overlay(alpha_spec.y, Desaturate(rawAlbedo.rgb, hairAlpha*g_ubPerMaterial._hairParams.w));
	// </Normal>

	// <LambertianScaleBias>
	float2 lamScaleBias = g_ubPerMaterial._lsb_gloss_lp.xy + float2(0, rawNormal.r*8.0*g_ubPerMaterial._lsb_gloss_lp.w);
	lamScaleBias = lerp(lamScaleBias, float2(1, 0.45), skinAlpha);
	// </LambertianScaleBias>

	// <Gloss>
	float gloss = lerp(g_ubPerMaterial._lsb_gloss_lp.z, g_ubPerMaterial._motionBlur_glossX.y, glossXAlpha);
	...
	DS_SetSpec(so, normalAA.w*max(eyeAlpha, max(strass,
		alpha_spec.y*(1.0 + hairAlpha * 0.75)*g_ubPerMaterial._ssb_as.x + g_ubPerMaterial._ssb_as.y)));
	DS_SetDepth(so, si.depth.x / si.depth.y);
