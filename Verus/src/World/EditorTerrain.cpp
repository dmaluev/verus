...
void EditorTerrain::ApplyBrushFlatten(const float xz[2], int radius, int strength, bool useNormal, PcVector3 pNormal, const float* pHeight)
{
	int ijCenter[2];
	ConvertToBufferCoords(xz, ijCenter);
	const glm::int4 rc = ComputeBrushRect(ijCenter, radius);
	...
	float centerHeight;
	short centerHeight16;
	if (pHeight)
	{
		centerHeight = *pHeight;
		centerHeight16 = Math::Clamp<int>(ConvertHeight(*pHeight), -SHRT_MAX, SHRT_MAX);
	}
	else
	{
		centerHeight = GetHeightAt(ijCenter, 0, &centerHeight16);
	}
	...
	for (int i = rc.y; i < rc.w; ++i)
	{
		for (int j = rc.x; j < rc.z; ++j)
		{
			...
			if (rr <= r2)
			{
				...
				short h;
				GetHeightAt(ij, 0, &h);
				short targetHeight16 = centerHeight16;
				if (useNormal)
				{
					const float h = centerHeight + slopeX * dj + slopeZ * di;
					targetHeight16 = Math::Clamp<int>(ConvertHeight(h), -SHRT_MAX, SHRT_MAX);
				}

				const int maxDelta = std::abs(targetHeight16 - h);

				int delta = 0;
				if (h > targetHeight16)
					delta = -std::abs(Math::Min(strength, maxDelta));
				else if (h < targetHeight16)
					delta = std::abs(Math::Min(strength, maxDelta));
	...
	TerrainSplatMode mode, const float* pMask, bool updateTexture)
{
	...
	{
		std::minstd_rand gen(ijCenter[0] + ijCenter[1]);
		for (int i = rc.y; i < rc.w; ++i)
		{
			for (int j = rc.x; j < rc.z; ++j)
			{
	...
	else
	{
		for (int i = rc.y; i < rc.w; ++i)
		{
			for (int j = rc.x; j < rc.z; ++j)
			{
				...
				{
					const int ij[2] = { i, j };
					const float alpha = 1 - sqrt(static_cast<float>(rr) / r2);
					...
					switch (mode)
					{
					case TerrainSplatMode::grad:
						falloff = alpha;
						break;
					case TerrainSplatMode::normal:
